...
def create_samples_and_fake_samples(path_1,path_2):
    with open(path_1) as json_file:
        x = json.load(json_file)
    a = []
    hd = None
    with open(path_2) as csvfile:#has the location/street info + link to the news articles
    ...
    return samples, false, obj, more, pre
########################

def rd():
  return np.random.rand()

def getTemplate(template, obj):
...
def createSentence(samples, obj, more, pre):
  text = ''
  if rd()>0.8:
   text+= getTemplate(choice(pre), obj)[0]
  
  template = choice(samples)
  #print(template)
  template, y = getTemplate(template, obj)
  text=''+template

  if rd()>0.8:
    template = choice(more)
    text+= ' '+getTemplate(template, obj)[0]
  Y = [];
  l = 0
  for i,j in y:
    Y.extend('-'*(i-l))
...
def create_many_Sentences(number_of_sentences, last_index, create_from_what, obj, more, pre):
    texts = []
    marks = []
    indexes = []
    for index, i in enumerate(range(0,number_of_sentences)):
        index = index + last_index
        x,y = createSentence(create_from_what, obj, more, pre)
        # print(x)
        # print(y)
    ...
    data = {'id': indexes,
           'y': 2, # 1 is for real phrases and 2 is for generated phraases
